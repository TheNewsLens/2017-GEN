...
export default class WikiMeta extends Component {
  ...
  render(){
    ...
    return (
      <div id={"video-meta-box-"+index} className={cx('video-meta-box','wiki',{'active': active, 'show': show})} onMouseEnter={this.onMouseEnter} onMouseLeave={this.onMouseLeave} onMouseMove={this.onMouseEnter}>
        ...
        <div className="video-type" onClick={this.onClick}>
          ...
          <div className="img" style={{backgroundImage: "url('/img/meta/wiki.png')"}}/>
        </div>
        <div id={"video-meta-"+index} className="video-meta wiki" onClick={this.toggle}>
          {/*<div className="wiki-logo"><img src="/img/wiki_logo.png"/></div>*/}
          <a className="link">{show ? 'HIDE' : 'SHOW MORE'}</a>
          <div className="title">{title}</div>
          <div className="text">{text}</div>
          <div className="long-text">{longText}</div>
